...
describe BinaryGame do
  describe '#max_guesses' do
    ...
    context 'when the range is 1 - 300' do
      it 'is 9 guesses' do
        max_game.instance_variable_set(:@range, [1, 300])
        max_guesses = max_game.max_guesses
        expect(max_guesses).to be(9)
      end
    end
  end
  ...
  describe '#computer_turns' do
    ...
    context 'when second guess is correct' do
      before do
        allow(turns_game.search).to receive(:make_guess).and_return(50, 25)
        allow(turns_game.search).to receive(:update_range).once
        allow(turns_game.search).to receive(:game_over?).and_return(false, true)
      end

      it 'sends make_guess twice' do
        expect(turns_game.search).to receive(:make_guess).twice
      end

      it 'sends update_range once' do
        allow(turns_game.search).to receive(:make_guess).twice
        expect(turns_game.search).to receive(:update_range).once
      end
    end
  end
end
